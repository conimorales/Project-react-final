...
function ItemListContainer( {greeting, items} ) {
    ...
    getProductos().then(respuestaPromise => {
        ...
        document.title = `Categoría ${categoryid}`;
   ...
   function setCatPath(){
    if (categoryid) {
        return <Container className="d-flex text-center justify-content-center mb-10 pb-10 align-items-center p-5"><Breadcrumb.Item linkAs={Link} linkProps={{ to: "/" }}>Productos</Breadcrumb.Item><Breadcrumb.Item active>{categoryid}</Breadcrumb.Item></Container>;
    }   else {
        return <Container className="d-flex text-center justify-content-center mb-10 pb-10 align-items-center p-5"><Breadcrumb.Item active>Productos</Breadcrumb.Item></Container>;
    }
}
